...
void lex(char *line, int start, int end, int tokenType){
   char token[MAXTOKEN];
   while(start < end && isspace(line[start]) && tokenType != 3){
      start++;
   }
   if(start >= end){
      return;
...
// Break the string down into tokens
void tokenize(char* line, int length, int* inComment) {
   int start = 0;
   int end = 0;
   while (end < length) {
      if((line[end] == '/' && line[end+1] == '*') || *inComment == 1){
         if(*inComment != 1){
            *inComment = 1;
         }
         while(end < length){
         ...
         line[length-1] = '\0';
         lex(line, start, end, 3);
         start = end;
     }
      else if (line[end] == '"' && start == end) {
         start = end;
         end++;
